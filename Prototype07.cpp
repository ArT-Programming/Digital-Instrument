...
public:
	...
	virtual void onSound(AudioIOData& io){	
		...
		float ratio = ((arduino[0].angleY + 90) / 180.) * 5;
		...
		while(io()){
			...
			float fc = 1000 * volume[0] + 1;
			float out[2];
			out[0] = fmSynth[0].modulate(fc, ratio, index) * volume[0];
			//out[1] = fmSynth[1].modulate(fc1, ratio1, index1) * volume[1];
			//out[1] = fmSynth[1].wavOsc(arduino[1].angleX, arduino[1].angleY);
			//fmSynth[1].clip(volume[1],1,0);
			float delay[2];
			float delayTime = (arduino[1].angleX / 90.) + 1;
			fmSynth[0].clip(delayTime,2,0);
			float feedback = (arduino[1].angleY / 180.) + 0.5;
			fmSynth[0].clip(feedback,1,0);
			
			delay[0] = fmSynth[0].echo(out[0], delayTime, feedback, 10000);
			...
			io.out(0) = out[0] + delay[0];
			io.out(1) = out[0] + delay[0];
